...
# 左右平均のRMS誤差を算出し，時系列とRMS棒グラフを表示する．
# さらに PCA の寄与率を算出し，プロットと Print 表示を行う．
...
file_L = '0406_2L.csv'
file_R = '0406_2R.csv'
delay_time = 0.0      # 左右端末の開始時刻ずれ補正 [s]
cod_time = 17.22       # 方向転換時刻 [s]
limit_min_time = 6.71
limit_max_time = 20.49

WINDOW_SIZE = 40      # PCA window
is_mask = 0           # プロット表示．1: 範囲内のみ表示．1以外: 全体表示
is_mask_g = 1         # 角度表示．1: -180~180° に正規化．1以外: 指定なし
...
# 範囲制限 limit_min_time ～ limit_max_time を適用
# =========================================================
...
# =========================================================
# 角速度データPCA法（第二主成分）
# CSV -> (time_s, theta_deg, pc1_ratio, pc2_ratio) を返す
# =========================================================
...
plot_heading_timeseries(
    heading_R_gyro_pca,
    heading_L_gyro_pca,
    title_str='時系列変化（角速度データPCA法）',
    ylabel_str='第二主成分軸とx軸正方向とのなす角 [°]'
)
...
# 4) 寄与率をPrint表示
# =========================================================
ratio_L = prepare_pca_ratio_df_for_plot(heading_L_gyro_pca, add_delay=True)
ratio_R = prepare_pca_ratio_df_for_plot(heading_R_gyro_pca, add_delay=False)
