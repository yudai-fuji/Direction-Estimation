#---PCAを用いて第一主成分の動きを矢印ベクトルで追うGif画像の出力(3次元から2次元)---
...
WINDOW_SIZE = 40
ARROW_LENGTH = 5
FILE_PATH = '0428_1R.csv'
# <--- 変更点: 出力ファイル名を変更 (quiverを使ったことが分かるように)
OUTPUT_GIF_PATH = FILE_PATH + '.gif'
...
print(f"リアルタイムFPS: {real_time_fps:.2f} (この値でGIFを保存します)")
...
ax.set_xlabel('Wx')
ax.set_ylabel('Wy')
ax.set_title('Growing Trajectory with PCA Direction (Origin Arrow)')
ax.grid(True)
...
def update(frame):
    ...
    #開始位置:0 終了位置frame+1(含まない)→ 0~frame番目
    growing_points.set_data(x_data[:frame+1], y_data[:frame+1])
    
    #frame <= 38，つまりデータ数が39個以下の場合
    ...
    data_window = np.column_stack((x_data[start_index:end_index],
                                   ...
                                   z_data[start_index:end_index]))

    #引数は2次元配列
    pca.fit(data_window)

    #第一主成分の方向ベクトルの配列[PCA1_x, PCA1_y, PCA1_z]を格納
...
#GIFとして保存
try:
    print("GIFの保存を開始します (PCA計算を含むため時間がかかる場合があります)...")
    #Gifと指定してない？
