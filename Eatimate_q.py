...
DO_PCA_RATIO_PLOT = True       # PCA寄与率プロット
...
# =========================================================
PLOT_ACC_PCA = True
PLOT_PROPOSED_ACC_PCA = True
PLOT_GYRO_INTEGRAL = True
PLOT_GYRO_PCA = True
...
# =========================================================
# 2つの角度を単位ベクトル平均する処理
# 引数: theta_R_deg，theta_L_deg は角度 [deg]
# 戻り値: ベクトル平均後の角度 [deg]
# =========================================================
def vector_mean_two_angles_deg(theta_R_deg, theta_L_deg):
    theta_R_rad = np.deg2rad(np.asarray(theta_R_deg, dtype=float))
    theta_L_rad = np.deg2rad(np.asarray(theta_L_deg, dtype=float))

    x_mean = (np.cos(theta_R_rad) + np.cos(theta_L_rad)) / 2.0
    y_mean = (np.sin(theta_R_rad) + np.sin(theta_L_rad)) / 2.0

    theta_mean_deg = np.rad2deg(np.arctan2(y_mean, x_mean))
...
def calc_rms_from_headings(heading_R, heading_L):
    ...
    t_eval = t_all[mask]
    theta_R_eval = theta_R_all[mask]
    theta_L_eval = theta_L_all[mask]
    theta_mean_eval = theta_mean_all[mask]
...
# =========================================================
# RMSE比較表の表示
# 列名は指定通り「左右平均RMSE」のまま
# 内部計算では左右平均にベクトル平均を使用
# =========================================================
